...
public class GetAllResourcesRouter {
    @Bean
    public RouterFunction<ServerResponse> getAll(GetAllResourcesUseCase getAllResourcesUseCase) {
        return route(
                GET("/api/resources").and(accept(MediaType.APPLICATION_JSON)),
                request -> ServerResponse.ok()
                        .contentType(MediaType.APPLICATION_JSON)
                        .body(BodyInserters.fromPublisher(getAllResourcesUseCase.getAllResources(), ResourceDTO.class))
        );
    }
}
